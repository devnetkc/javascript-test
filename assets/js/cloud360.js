    ...
    for (const Emp of Employees) {
        try {
            ...
            const Results = await Promise.all([
                ...
                Q2_SetBestCustomer(EmpSales, CurrCommYear),
            ]);
            Emp.daystobday = Results[0];
            RenderResults(Emp, $('#result-Q1'));
            Emp.bestcustomer = Results[1];
            RenderResults(Emp, $('#result-Q2'));
 ...
 */
const Q2_SetBestCustomer = async empSales => {
    try {
        ...
        if (empSales.length <= 0) {
            ...
            return { customer: 'N/A', totalsales: 0 };
        }
         ...
         */
        const BestCustomer = CustomerSaleTotals.reduce(
            (prevTrans, curTrans) => {
        ...
        return { name: BestCustomer.customer, totalsales: BestCustomer.amount };
    } catch (err) {
        console.error(err);
    }
};
...
const RenderResults = (content, selector) => {
    console.log(
        `rendering here ${JSON.stringify(selector)} this content: ${JSON.stringify(
      content
    )}`
    );
